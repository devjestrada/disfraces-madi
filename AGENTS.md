...
8. **Confirmación de cambios:** Al finalizar la implementación, preguntar al usuario si los cambios se aplicaron correctamente.
   - Si el usuario **confirma** que todo está correcto, dentro de la misma rama `feature`:
     a. Hacer commit de los cambios de código (commit funcional, ej. `feat: ...` o `fix: ...`).
     b. Mover el archivo de la spec a `docs/specs/specs_done/`, subir (incrementar) la versión en `package.json` (`npm version`) y actualizar `CHANGELOG.md` (mover el contenido de `[Unreleased]` a la nueva versión). Hacer commit de este cierre documental por separado (ej. `chore: cierre de spec nombre-de-la-spec (vX.Y.Z)`), manteniéndolo distinto del commit funcional aunque ambos vivan en la misma rama.
     c. Subir (`push`) la rama `feature` al repositorio remoto y abrir un **único Pull Request** hacia la rama principal que incluya tanto el código funcional como los commits de cierre (SemVer, Changelog y movimiento de spec).
...
11. **Cierre de rama:** Una vez que el Pull Request sea aprobado y mergeado a la rama principal, borrar la rama `feature` correspondiente tanto en local (`git branch -d`) como en el remoto (`git push origin --delete`), para mantener el repositorio limpio. No borrar la rama mientras su PR no haya sido aprobado y mergeado.
...
- Cada spec finalizada exitosamente debe resultar en **un único Pull Request** (rama `feature`) que incluya, como commits separados, tanto el código funcional como el cierre documental (movimiento de la spec a `specs_done`, bump de versión en `package.json` y entrada en `CHANGELOG.md`). No se abren PRs adicionales para el cierre de una spec.
- Los cambios que sean exclusivamente de documentación **ajenos a una spec en curso** (por ejemplo, edición de `AGENTS.md`, `CLAUDE.md`, `README.md`, `DESIGN.md` u otros archivos `.md` que no formen parte del cierre de una spec) **nunca** deben mezclarse en la misma rama que cambios de código funcional. Siempre deben ir en una rama separada (ej. `docs/nombre-del-cambio` o `chore/nombre-del-cambio`), con su propio commit, push y Pull Request independiente.
